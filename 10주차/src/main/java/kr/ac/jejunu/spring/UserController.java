...
public class UserController {
    ...
    //ModelAttribute
    @GetMapping //얘도 해당클래스최상위의 viewname찾으러감
...
//    @PostMapping  // 요거는 위의 거랑 구분이 됨
//    public void user(User user){
//    }

    @GetMapping("/string")  //이경우에는 리턴되는 스트링의 이름을갖는 뷰네임이
    public String returnStringTest(){
        return "user";
    }

    @GetMapping("/redirect")    //redirect는 url을 바꿔주는것. >> 특정주소로 바꿔주는것
    public String redirectStringTest(){
        return "redirect:/user";
    }

    @GetMapping("/forward")    //forward url은 그대로인데 내용물은 바꿔준 url의 내용이 뜨는것.
